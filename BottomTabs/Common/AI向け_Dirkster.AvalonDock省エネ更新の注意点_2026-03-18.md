...
1. `先に結論`
   - 何を守る資料なのかを先に掴む。
2. `AvalonDock で意識する状態`
   - `IsVisible / IsSelected / IsActive` の意味差を確認する。
3. `省エネ更新で分けるべき 3 種類`
   - ポーリング、イベント更新、初回レイアウトを分けて考える。
4. `よくある失敗`
   - 今の不具合が既知パターンかどうかを当てる。
5. `このリポジトリでの実装指針`
   - どのファイルを見るかを決める。

## 役割サマリ

- この資料は `BottomTabs/Common/BottomTabActivationGate.cs` まわりの判断基準を共有する。
- 主に「非アクティブ時は止めるが、再表示時に古く見せない」ための guardrail を置く。
- 実装そのものではなく、`gate / dirty / timer / layout` の使い分けを揃えるための資料と考える。
...
- 下部タブ資料も、まず `役割 / 読み順 / 注意点` の順で読める形を保つ。
