...
### 3.5 WiX installer v1 の追加

Public repo 側には、verify 済み app package を唯一入力にする `WiX v6` installer 導線が追加されている。
...
- Public release は `ZIP` に加えて `bundle exe` も配る構造へ進んだ
- ただし 2026-04-05 時点の live proof は `v1: install / upgrade / uninstall` までである
- `self-update` は `v2`
- `custom BA` の保持項目 UI は `v3`

つまり fork 元統合時は、

- `WiX installer は入っている`
- ただし current scope は `v1 proof`
...
- Public repo は verify 済み app package を入力に `WiX bundle exe` も組み立てる
...
- verify 済み app package を入力に WiX installer を組み立てる
...
- installer の current scope は `v1: install / upgrade / uninstall`
- `self-update` と `custom BA` は後続段階であり、fork 元統合時点で完了済み前提にはしない
