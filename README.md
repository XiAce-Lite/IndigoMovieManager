...
## 高速化：FFmpeg.AutoGen採用
- サムネイル生成エンジンとして `FFmpeg.AutoGen` を採用。
- 目的はフレーム取得の高速化（高速シーク）です。
- フォーク先リポジトリの制限により、Git LFS の新規オブジェクト登録ができないため、必要DLLは通常オブジェクトとして管理しています。
- ただし `FFmpeg.AutoGen 8.0.0` は `62系DLL` を要求し、`avcodec-62.dll` が100MB超で push 制限に抵触します。
- そのため本フォークでは `FFmpeg.AutoGen` を `7.1.1` に固定しています。
