...
- 正本は `AI向け_ブランチ方針_ユーザー体感テンポ最優先_2026-04-07.md` と `AI向け_現在の全体プラン_2026-04-07.md` とする。
...
- 2026-03-30 追記で、default rescue slot 側でも enqueue 後の delayed recheck と periodic sync からの rescue kick を入れ、`等間隔サムネイル作成` を含む「受付済みだが実処理が始まらない」経路を減らした。
- 2026-03-30 追記で、単動画のユーザー指示 job は `manual` 差し込み扱いへ寄せた。`等間隔サムネイル作成` と下部 `サムネ失敗` タブの選択救済も、1件選択時は一般待ちより manual slot を優先する。
