...
- 同梱 worker / published artifact の marker
- `project-build` fallback を local 開発時だけ明示 opt-in で許可する設定
...
- `sha256` が一致する
- `project-build` fallback は既定で無効であり、必要時だけ明示 env で有効化する

起動後に `result.json` を読む時も、次を fail-fast で確認する。

- `contractVersion` が一致する
- `mode` が `rescue-main` である
- `requestId` が空でない
- `requestId` が起動時に発行した job と一致する
