...
- Private repo の publish 結果を消費する consumer 側処理であり、残す方が自然である
...
### 4.1 local worker source build

- `C:\Users\na6ce\source\repos\IndigoMovieManager\scripts\create_github_release_package.ps1`
- `C:\Users\na6ce\source\repos\IndigoMovieManager\scripts\create_rescue_worker_artifact_package.ps1`
- `C:\Users\na6ce\source\repos\IndigoMovieManager\scripts\invoke_release.ps1`
- `C:\Users\na6ce\source\repos\IndigoMovieManager\.github\workflows\rescue-worker-artifact.yml`

扱い:

- 既定では無効
- `-AllowLocalWorkerSourceBuild` または対応 env を与えた時だけ使う

理由:

- 2 repo 同時開発や emergency 切り分けではまだ価値がある
- ただし Public repo の正本道線ではない

### 4.2 project-build fallback
...
### 4.3 worker 実装直結 test

- `C:\Users\na6ce\source\repos\IndigoMovieManager\Tests\IndigoMovieManager.Tests\IndigoMovieManager.Tests.csproj`
- `C:\Users\na6ce\source\repos\IndigoMovieManager\Tests\IndigoMovieManager.Tests\RescueWorkerApplicationTests.cs`

扱い:

- 既定では不参加
- `ImmIncludeWorkerSourceTests=true` の時だけ local opt-in

理由:

- Private repo 側 test へ移すまでの橋渡しとしてはまだ価値がある
- ただし Public repo の既定 test セットには含めない
...
- Public repo から Private repo seed を再生成・同期する入口だからである
- 外だしの移行期が終わるまで、完全に不要にはならない
...
1. `RescueWorkerApplicationTests.cs` の Private repo への完全移送
2. `IndigoMovieManager.Tests.csproj` の `ImmIncludeWorkerSourceTests` 条件ごと削除
3. `rescue-worker-artifact.yml` の役割を Private repo 運用で十分代替できるか再判断
4. local worker source build opt-in を、数回の安定 release 後にさらに縮退できるか見直す
...
4. 残る worker source 直参照が「明示 opt-in」「bootstrap」「履歴資料」のどれかに分類できる
