...
- ただし Public repo の正面入口 `invoke_release.ps1` と `create_github_release_package.ps1` では使わせず、worker artifact 側の下位 script だけに閉じ込める
- さらに下位 script 側でも warning を出し、正本道線ではないことを明示する
...
1. local worker source build opt-in を、数回の安定 release 後にさらに縮退できるか見直す
