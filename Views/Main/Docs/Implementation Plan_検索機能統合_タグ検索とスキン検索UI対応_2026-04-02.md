# Implementation Plan: 検索機能統合 かな・ローマ字・タグ検索とスキン検索UI対応 2026-04-02

最終更新日: 2026-04-07

変更概要:
- かな・ローマ字検索を「これから追加する機能」ではなく「既にある検索仕様」として再定義
- `SearchService` の責務を、単純なフィルタ処理から「検索文字列解釈 + 比較キー正規化 + fallback 統合」へ広げた
- `Roma` を `Kana` から都度導く補助値ではなく、DB正本列を優先しつつ空時だけ fallback する扱いに修正
- ユーザー体感テンポを守るため、検索時の都度変換を避けるキャッシュ/比較キー方針を追記
- public 構文は広げ過ぎず、タグ専用構文だけを明示し、かな・ローマ字は通常検索へ自然統合する方針に整理

## 1. 目的
- 検索機能を `MainWindow` 内の複数箇所に散らばった状態から整理し、検索仕様の正本を 1 か所へ寄せる。
- 既に実装済みの `Kana` / `Roma` 検索を、場当たり的な追加条件ではなく、本体検索仕様の一部として固定する。
...
### 2.2 かな・ローマ字検索が既存仕様になり、切り出し対象が増えた
- 現状の検索対象は `Movie_Name` / `Movie_Path` / `Tags` / `Comment*` に加え、`Kana` / `Roma` まで広がっている。
- `Kana` はひらがな正本だが、検索ではカタカナ入力も吸収する必要がある。
- `Roma` は DB 列を優先しつつ、空時は `Kana` や `Movie_Name` / `Movie_Path` から fallback する必要がある。
- つまり今や検索の難所は「タグ構文」だけではなく、「比較キー正規化と fallback」そのものにある。

### 2.3 タグ検索が「通常検索の一部」に埋もれている
- 現状でも `Tags` は通常検索対象に入っている。
- しかし、タグ専用検索の責務が分離されておらず、WB 互換 `!tag` や `tagbar` とつなぎにくい。

### 2.4 スキン検索 UI の受け口がまだ定義し切れていない
- スキンチーム前提は次である。
  - スキンは検索 UI を持てる
  - 検索実行は `wb.*` で本体へ依頼する
...
### 3.2 かな・ローマ字は通常検索へ自然統合する
- `Kana` / `Roma` はタグのような専用構文を増やすより、まず通常検索の対象として自然に効くことを優先する。
- かな・ローマ字専用の public 構文は、当面は追加しない。
- 将来必要になっても、内部のクエリ表現だけ先に一般化し、外部構文は慎重に増やす。
...
### 3.5 ただし検索時の都度変換は増やさない
- `JapanesePhoneticAnalyzer` やローマ字 alias 生成を、検索のたびに全件へ繰り返す設計は採らない。
- DB にある `Kana` / `Roma` を最優先で使い、空時 fallback だけを補助的に使う。
- fallback が必要な値は、検索 service 内で比較キーとして一度だけ正規化し、同一検索内またはセッション内で再利用する。
...
### 4.2 比較キー生成の内部責務
- `SearchService` の内部、または補助クラスとして「検索比較キー生成」を持つ。
- ここでは次を統一する。
  - 通常文字列の大文字小文字非依存比較
  - `Kana` のひらがな/カタカナ吸収
  - `Roma` の alias 吸収
  - `Tags` の改行区切り正規化
  - DB 空値時の fallback
- この責務を切り出さないと、`FilterMovies(...)` の移設後に挙動差分が起きやすい。

### 4.3 `SearchExecutionController`
- 役割
  - `SearchKeyword` 更新
  - `FilterAndSort(...)` 呼び出し
  - `SearchCount` 反映
...
- 対象フィールド
  ...
  - `Comment3`
- 比較ルール
  - `Kana` は DB ではひらがな正本を使い、検索ではカタカナ入力も比較用正規化で吸収する
  - `Roma` は DB 列を優先して使う
  - `Roma` 列が空でも `Kana` または `Movie_Name` / `Movie_Path` からの fallback を許容する
  - ローマ字は完全一致ではなく、既存実装の alias も検索対象へ含める
- 継続仕様
  - フレーズ検索
  - OR
  - NOT
...
これらは内部表現を一般化してから、必要性が確認できた時だけ追加する。
...
### Phase 0: 現行検索仕様の固定
- 既存の `MainWindowViewModel` 検索挙動をテストで先に固定する
- 対象
  - かな検索
  - カタカナ入力吸収
  - ローマ字検索
  ...
  - `{notag}`
  - `{dup}`
- ここで「現状互換」を先に押さえ、後続の service 抽出で壊さないようにする

### Phase 1: 検索判定ロジックの切り出し
- `SearchService` 追加
- 比較キー正規化と fallback を service 側へ移す
...
### 9.2 かな・ローマ字 fallback がホットパスを重くするリスク
- 回避策
  - DB にある `Kana` / `Roma` を最優先で使う
  - 空値時だけ fallback を使う
  - 比較キーは同一検索内で再生成しない
  - backfill により長期的に fallback 依存を減らす

### 9.3 タグ構文を急に増やして既存検索を壊すリスク
- 回避策
  - 初期の正本は `!tag:` に固定する
  - `!猫` 互換は後段で opt-in 的に追加する

### 9.4 スキン側へ検索ロジックが漏れるリスク
- 回避策
...
- かな・ローマ字検索の現行挙動が service 化後も保たれている
