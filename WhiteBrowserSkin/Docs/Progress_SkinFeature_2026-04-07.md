...
- 2026-04-22: `umiFindTreeEve` の runtime bridge 実 host における `onSkinLeave -> wb.changeSkin("MissingSkin") -> wb.changeSkin("#TagInputRelation")` は、`failure 単体` と `success 単体` は green でも、直列では最初の `MissingSkin` 結果待ちが timeout する。直列遷移の未固定境界として分離して扱う。
