...
- 2026-04-12: `wb.getRelation` の最小実装を追加し、visible movies からタイトル近傍とタグを返せるようにした。`TagInputRelation` 向けに compat 側へ `movieInfoCache` / `relationCache` と `onExtensionUpdated` 前 prefetch を入れ、実 build 出力 skin で候補タグ生成まで確認した。
...
- 2026-04-12: `umiFindTreeEve` は build 出力 skin の runtime bridge 統合テストで、`onSkinEnter` から `Folders` / `Tags` tree と footer (`ClearCache` など) を生成できるところまで確認した。これにより、当初未表示だった 6 本のうち extension 系は `TagInputRelation` が候補タグ生成、`umiFindTreeEve` が tree / footer 生成まで前進した。
...
- 2026-04-17: `Alpha2` は MainWindow 実 host で `wb.selectThum(77, true)` と `wb.selectThum(84, true)` の受け入れも確認した。`wb.getSelectThums()` と WPF 側の複数選択が `77 / 84` で揃い、focus は `77` のまま維持され、focused row は `focus` class に加えて `thum_select` も持つ形で focus/select 強調が共存することを固定した。
