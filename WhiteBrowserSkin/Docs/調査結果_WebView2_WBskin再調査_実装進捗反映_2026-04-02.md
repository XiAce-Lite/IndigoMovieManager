...
- MainWindow 実 host でも `WhiteBrowserDefaultGrid` / `Small` / `Big` の config 駆動 `seamless-scroll` 追記を確認した
...
- MainWindow 実 host でも `WhiteBrowserDefaultGrid` / `Small` / `Big` の `seamless-scroll` 追記後 `find("Movie201", 0)` の先頭復帰を確認した
