...
- minimal host chrome
- `wb.update / find / sort / getInfo / getInfos / getProfile / writeProfile / changeSkin / focusThum / selectThum / scrollTo`
...
今回の作業で第一段は入ったが、まだ厚みを増やす余地がある。

- `onUpdateThum`
- 実ホストの DB 切替 / skin 切替経路を含む lifecycle 検証強化
...
### 5.3 次の仕事は tag / `onUpdateThum` / 大量件数対策

本当に残っている根幹は次である。

- tag 系 API
- `onUpdateThum` と実ホスト lifecycle 検証
- 大量件数時の差分更新と可視範囲最適化
...
1. tag 系 API
2. `onUpdateThum` と実ホスト lifecycle 検証
3. 大量件数対策
...
- tag をどこまで WB 的に触れるか
- `onUpdateThum` と実ホスト lifecycle をどこまで無修正互換へ寄せるか
- 大量件数でもテンポを維持できるか
