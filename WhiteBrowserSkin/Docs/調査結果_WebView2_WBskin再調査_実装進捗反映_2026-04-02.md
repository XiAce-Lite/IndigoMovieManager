...
- `SimpleGridWB` は MainWindow 実 host で、`200:60` の残り件数だけ要求し、既存 card DOM を保持したまま追記できることも確認した
- `SimpleGridWB` は MainWindow 実 host で、offscreen thumb を保留し、scroll 後に末尾 thumb を `src` へ昇格できることも確認した
- `SimpleGridWB` は MainWindow 実 host で、`onUpdateThum(recordKey, thumbUrl)` により offscreen thumb の `data-thumb-url` と state を差し替え、scroll 後に新しい `src` へ昇格できることも確認した
- `SimpleGridWB` は MainWindow 実 host で、可視範囲から外れた先頭 thumb を `src` から `data-thumb-url` へ戻し、末尾 thumb の昇格と両立できることも確認した
- `SimpleGridWB` は MainWindow 実 host で、可視範囲外 card に `is-distant` を付けて本文詳細を休ませ、scroll 後に先頭 card と末尾 card の状態が入れ替わることも確認した
- compat runtime は、`onCreateThum` 未実装の外部 skin に最小サムネ生成 fallback を持たせ、`onUpdateThum` 未実装でも `img{id}` / `#thum{id} img` / `data-record-key` へ既定 `src` 差し替えを行えるようにした
...
- compat runtime の `seamless-scroll` も、残り件数が分かる時は `update(startIndex, remainingCount)` だけを要求するようにした
- `MainWindowWebViewSkinIntegrationTests` は 42/42、`WhiteBrowserSkinRuntimeBridgeIntegrationTests | WhiteBrowserSkinCompatScriptIntegrationTests | WhiteBrowserSkinEncodingNormalizerTests | WhiteBrowserSkinRenderCoordinatorTests` は 29/29 を確認した
- 上記構成へ整理したあと、combined broad も 71/71 通過を確認した
- compat script 統合テストで、callback 未実装の最小 harness でもサムネ生成と後追い差し替えが通ることを追加確認した
- `WhiteBrowserDefaultList` 実 fixture でも、既定 `onUpdateThum` fallback による `img77` の `src` 差し替えを確認した
...
- 当初未表示だった 6 本は全件で初回サムネ表示を通過した
- `TagInputRelation` は候補タグ生成まで確認した
- `umiFindTreeEve` は `onSkinEnter` から `Folders` / `Tags` tree と footer 生成まで確認した
- `#TagInputRelation` / `#umlFindTreeEve` のような `#` 付き skin フォルダは、`skin.local` の base href を URL encode しないと相対 JS/CSS が fragment 扱いで崩れることを確認し、実装修正済み
- 旧 sync 前提の `getDBName` / `getInfos(0,-1,...)` / `readFile` / `writeFile` / `getWatchList` は compat shim で段階吸収できるところまで進んだ
