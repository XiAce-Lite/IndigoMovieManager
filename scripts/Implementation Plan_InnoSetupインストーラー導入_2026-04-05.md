...
変更概要:
- Inno Setup を使ったインストーラー / アンインストーラー導入計画を整理
...
- Inno Setup を installer shell に使う
...
- 今回の本命は `verify 済み app package` の後段へ `Inno Setup` を薄く載せ、`uninstall で何を残し、何を消してよいか` を事故らない粒度で固定することである
...
- 実装順は `staging -> install -> uninstall保持選択 -> release統合` を v1 とし、`自己更新基盤` は v2 で続けるのが安全である
