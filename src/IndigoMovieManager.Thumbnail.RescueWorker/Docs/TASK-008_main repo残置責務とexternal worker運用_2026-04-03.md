...
これは既定導線ではなく、local 開発と切り分け用の例外である。

### C. 引退済みの橋渡し資産

- `scripts/bootstrap_private_engine_repo.ps1`
- `scripts/private-engine-seed/`

これは runtime 密結合ではなく、Public repo から Private repo seed を再生成する橋渡しとして使っていた。
2026-04-05 時点で、Private repo clone + Private repo docs / scripts が正面入口として成立したため引退した。
したがって current な main repo 残置責務ではない。
...
同日、Public repo 側 bootstrap / seed も引退させ、Private repo clone を唯一の初期化入口へ寄せた。
